...
export function StatsCounter({ stats, translations, className = '' }: StatsCounterProps) {
    ...
    const gridCols = stats.length === 2 ? 'lg:grid-cols-2 max-w-2xl mx-auto' :
                     stats.length === 3 ? 'lg:grid-cols-3 max-w-4xl mx-auto' : 'lg:grid-cols-4';

    return (
        <ul
            ref={containerRef}
            className={`stats-counter grid grid-cols-1 sm:grid-cols-2 ${gridCols} gap-4 lg:gap-6 list-none p-0 m-0 ${className}`}
